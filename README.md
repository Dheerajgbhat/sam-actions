...
```bash
sam-action$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-action$ sam local start-api
sam-action$ curl http://localhost:3000/
```
...
```bash
sam-action$ sam logs -n HelloWorldFunction --stack-name sam-action --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam-action
```
